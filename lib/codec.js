...
// These allow the old underscore-prefixed function names to still work
async function _ae(txt, p) { return Codec.encrypt(txt, p); }
async function _ad(d, p) { return Codec.decrypt(d, p); }
async function deriveRoomId(p) { return Codec.deriveRoomId(p); }
function _pk(p) { return Codec._pack(p); }
function _up(b) { return Codec._unpack(b); }
function _toHex(buf) { return Codec._toHex(buf); }
function _fromHex(hex) { return Codec._fromHex(hex); }
function _b2B(bits) { return Codec._bitsToBytes(bits); }

// ── Exports ────────────────────────────────────────────────────────────────────

export { Codec, Enc, deriveRoomId, _ae, _ad, _pk, _up, _toHex, _fromHex, _b2B };
export { PBKDF2_ITERATIONS as _PI, SALT_BYTES, IV_BYTES, PACKET_PREFIX as _BRN, VERSION as _VR };
